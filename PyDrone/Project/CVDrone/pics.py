...
TARGET_COLOR_MIN = np.array([0,100,100], np.uint8) # Green -> 35,50,50; Pink? -> 120,50,50; Blue -> 110 or 0; Skin Color -> 0
TARGET_COLOR_MAX = np.array([5,255,255], np.uint8) # Green -> 60,255,255; Pink? -> 180,255,255; Blue -> 130 or 15; Skin Color -> 120

def main():
	W, H = 1280, 720 # 640, 360 for not HD
	screenMidX = W/2
	screenMidY = H/2
	circleIncrement = H/12
	...
	while running:
		try:
			...
			if pixelarray != None:
				...
				#allCont = frame
				#cv2.drawContours(allCont, contours, 0, (0, 0, 255), -1)
				...
				if len(areas)>0:
					...
					diff = circleIncrement # Increment of radius of the Speed Limit Circles
